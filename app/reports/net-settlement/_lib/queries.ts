...
export async function getNetSettlements(
  input: GetNetSettlementParams
): Promise<NetSettlementApiResponse> {
  ...
  try {
    const requestUrl = `${
      process.env.API_URL
    }/reports/net-settlement?${buildNetSettlementQueryString(input)}`;
    ...
    return data as NetSettlementApiResponse;
  } catch (error) {
    console.error(error);
    return {
      res: "error",
